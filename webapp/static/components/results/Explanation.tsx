...
const Explanation = ({ flags, detected }: ExplanationProps) => {
    const activeFlags = Object.entries(flags).filter(([, value]) => value).map(([key]) => key);
        ...
        <div className="bg-gray-800/70 border border-gray-700 rounded-lg p-4 mt-4">
            <div className="flex items-center gap-2 mb-2">
                <span className={`px-2 py-1 rounded text-xs font-semibold ${
                    detected ? 'bg-red-600/30 text-red-200' : activeFlags.length ? 'bg-yellow-600/30 text-yellow-200' : 'bg-green-700/30 text-green-200'
                }`}>
                    {detected ? 'Vulnerability' : activeFlags.length ? 'Flagged' : 'Safe'}
                </span>
                {activeFlags.length > 0 && (
                    <span className="text-xs text-gray-300">Flags: {activeFlags.join(', ')}</span>
                )}
            </div>
